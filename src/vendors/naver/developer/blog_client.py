...
logger = get_logger("vendors.naver.blog")


class NaverBlogClient(NaverSearchClient):
    """네이버 블로그 검색 API 클라이언트"""
    
    def __init__(self):
        super().__init__(SearchType.BLOG.value, rate_limit=1.0)
        self.api_info = get_api_info(SearchType.BLOG)
    ...
    def search_blogs(self, 
        ...
        """
        # 정렬 옵션 유효성 확인
        if not validate_sort_option(SearchType.BLOG, sort):
        ...
        try:
            ...
            logger.debug(f"네이버 블로그 API 성공: {data.get('total', 0)}개 결과")
            return data
            
        except Exception as e:
            logger.error(f"네이버 블로그 API 호출 실패: {e}")
...
# 전역 클라이언트 인스턴스
naver_blog_client = NaverBlogClient()